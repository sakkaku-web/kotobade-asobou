...
export const StatsModal = ({
  ...
  if (displayLanguage === PREFERRED_DISPLAY_LANGUAGE) {
    statsModalTitle =
      now.setLocale('ja-JP').toLocaleString(DateTime.DATE_FULL) +
      ' 第' +
      solutionIndex.toString() +
      '回';
  } else {
    statsModalTitle =
      'Game #' +
      solutionIndex.toString() +
      ' on ' +
      now.setLocale('en-US').toLocaleString(DateTime.DATE_FULL);
  }

  const linkClassName = classnames(
    isHighContrast ? 'text-orange-600' : 'text-green-600',
    'underline text-sm'
  );
    ...
    >
      ...
      {(isGameLost || isGameWon) && (
        <div className="flex gap-1 justify-center text-sm dark:text-white mx-1">
          {t(
            'YESTERDAY_CORRECT_WORD_MESSAGE',
            yesterdaySolutionIndex.toString()
          )}
          <a
